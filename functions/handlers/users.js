...
//this is a function for signup user
exports.signup = (req,res) => {
    ...
    //give a default image for a new profil
    const noImage = 'noImage.png';
    let token, userId;
    db.doc(`/user/${newUser.handle}`).get()
      .then(doc => {
          if(doc.exists){
              return res.status(400).json({handle: 'his handle is already taken'});
      ...
      .catch(err => {
          console.error(err);
          if(err.cod=== 'auth/email-already-in-use'){
...
// it's a function for upload image 
...
exports.uploadImage = (req, res) => {
    ...
    busboy.on('file', (fieldname, file, filename, encoding, mimetype) => {
      ...
      // emna123.png bch t9asmo ['emna123','png']
